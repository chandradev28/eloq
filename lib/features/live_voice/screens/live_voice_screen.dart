...
class _LiveVoiceScreenState extends ConsumerState<LiveVoiceScreen>
  ...
  void initState() {
    ...
    if (widget.session != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted) return;
        ref.read(liveVoiceProvider.notifier).restoreSession(widget.session!);
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(liveVoiceProvider);
    final controller = ref.read(liveVoiceProvider.notifier);
                  ...
                  child: Row(
                    ...
                    children: [
                      _CircleActionButton(
                        ...
                        tooltip: state.isSessionActive || state.isConnecting
  ...
  Future<void> _openLiveSetup(
    ...
    LiveVoiceController controller,
                      ...
                      children: [
                        ...
                        TextField(
                          ...
                          decoration: const InputDecoration(
                            hintText:
                                'Example: Keep replies warm, short, and focused on pronunciation.',
                          ),
                        ),
                        const SizedBox(height: 20),
                        Text(
...
class _LiveIdleStage extends StatelessWidget {
  const _LiveIdleStage({
    super.key,
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        children: [
          const Spacer(flex: 2),
          _StatusGlass(label: statusLabel),
...
class _LiveTranscriptStage extends StatelessWidget {
  const _LiveTranscriptStage({
    super.key,
    required this.state,
    required this.pulse,
    required this.onOrbTap,
      ...
      children: [
        ListView(
          ...
          children: [
            ...
            if (state.userDraft.trim().isNotEmpty)
              Align(
            ...
            if (state.assistantDraft.trim().isNotEmpty)
              Padding(
            ...
            if (state.isConnecting)
              const Padding(
                ...
                child: TypingIndicator(label: 'Eloq is connecting...'),
              ),
          ],
        ),
        Align(
